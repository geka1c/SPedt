...
Функция РедактироватьНаСервере()   
	...
	Если СокрЛП(Комментарий) = "" Тогда 
		текстСообщения = текстСообщения + "Необходимо заполнить комментарийи" + Символы.ПС;
		
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Сотрудник) Тогда 
		текстСообщения = текстСообщения + "Считайте штрихкод сотрудника" + Символы.ПС;
	КонецЕсли;                                                      
  ...
  нз = РегистрыСведений.ИзмененияДокументов.СоздатьМенеджерЗаписи();	
	//нз 					= РегистрыСведений.ИзмененияДокументов.СоздатьНаборЗаписей();
	нз.Документ 		= Документ;
	нз.ДатаИзменения 	= ТекущаяДата();
	нз.Сотрудник 		= Сотрудник;
	нз.Описание 		= Комментарий;
	нз.Записать();      
	
	УстановитьПривилегированныйРежим(Истина);

	ПараметрыСеанса.РазрешитьИзменения = Документ;
...
Процедура Редактировать(Команда)
	Если РедактироватьНаСервере() Тогда
		Закрыть(ДанныеШК)		
	КонецЕсли	
КонецПроцедуры
