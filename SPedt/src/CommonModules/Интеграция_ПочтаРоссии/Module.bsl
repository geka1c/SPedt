...
Функция СозданиеЗаказаНаДоставку(Параметры) Экспорт
	...
	КонецЕсли;
	РасчетПоСтарымТарифам = (Параметры.ДокументОтправления.Коробка.ВерсияТарифов = "0");
	Если РасчетПоСтарымТарифам Тогда
		Данные.Вставить("insr-value", 			0);//Параметры["ОбъявленнаяСтоимость"]);								//Сумма объявленной ценности (копейки) 
	иначе	
		Данные.Вставить("insr-value", 			100);//Параметры
	КонецЕсли;	
...
//			Результат.transport_type = "EXPRESS";
	...
	Иначе
		Если ЭтоПочтаРоссии Тогда
			Результат.transport_type = "SURFACE";
			Результат.mail_category  = "WITH_DECLARED_VALUE";
...
Функция РасчетСтоимости(Параметры) Экспорт
	...
	РасчетПоСтарымТарифам = (Параметры.ДокументОтправления.Коробка.ВерсияТарифов = "0");
	Если РасчетПоСтарымТарифам Тогда
		Данные.Вставить("declared-value", 			0);//Параметры["ОбъявленнаяСтоимость"]);								//Сумма объявленной ценности (копейки) 
	иначе	
		Данные.Вставить("declared-value", 			100);//Параметры
	КонецЕсли;		
