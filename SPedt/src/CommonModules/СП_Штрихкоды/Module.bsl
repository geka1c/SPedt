...
Функция ПолучитьДанныеПоШК(ШК)	Экспорт
	...
	Если 		СтрНачинаетсяС(ШК,"12") Тогда
		...
		Если ДанныеШК = Неопределено Тогда Возврат ДанныеШК ; КонецЕсли;
...
Функция ПолучитьДокументВыдачаТранзита(ШКод)
	Если СтрНачинаетсяС(ШКод,"70")  Тогда 
		Префикс			= Константы.ПрефиксУзлаРаспределеннойИнформационнойБазы.Получить();
		НомерДокумента 	= ?(префикс = "","00",префикс)+"-"+Прав(ШКод,СтрДлина(ШКод)-2);
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Не корректный ШК Документа ""Расходная"". ШК = """+ШКод+"""");
	Возврат Неопределено;
КонецФункции	
...
функция ДополнитьШтрихКодПроверочнымКодом(ШКод)
	СуммаСимволов	=0;
	СтрокаЧтения 	= ШКод;
	...
	ШК_Строка 		= ШКод+?(РасщитанныйКод=0,"00",формат(РасщитанныйКод,"ЧЦ=2; ЧН=0; ЧВН=; ЧГ=0"));
	Возврат ШК_Строка;
КонецФункции	

Функция КорректныйШК(ШКод,ПроверочныйКод)
	СуммаСимволов=0;
	СтрокаЧтения = ШКод;
