...
&НаКлиенте
Процедура ТрекНомераШтрихКоды(Команда)
	Отправления = новый Массив();		
	Для Каждого элем из элементы.ТрекНомера.ВыделенныеСтроки Цикл
		стрТрекНомер = Объект.ТрекНомера.НайтиПоИдентификатору(элем);
		Отправления.Добавить(стрТрекНомер.ВнутреннийНомер);
	...
	Протоколы = ТрекНомераШтрихКодыНаСервере(Отправления);
	
	Для каждого протокол из Протоколы Цикл
		новыйПротокол = Объект.ПротоколыПередач.Добавить();
		ЗаполнитьЗначенияСвойств(новыйПротокол,протокол);
		Если протокол.Результат = "Получен штрихкод" Тогда
		//	ИмяФ=ПолучитьИмяВременногоФайла(протокол.ТипФайла);
			ПолучитьФайл(протокол.АдресФайла, СтрШаблон("Штрихкод_%1.%2",Объект.Коробка,протокол.ТипФайла), Истина);
		//	ЗапуститьПриложение(ИмяФ);
		КонецЕсли;	
	КонецЦикла;
КонецПроцедуры
&НаСервере
Функция  ТрекНомераШтрихКодыНаСервере(Отправления)
	

	Если Объект.Тип="sdec"  или Объект.Тип = "sdecCourier" или Объект.Тип = "postamat"  Тогда
	
		Протоколы = Интеграция_СДЭК_в20.ФормированиеШтрихкодовКЗаказу(Отправления);
...
&НаКлиенте
Процедура ТрекНомераОчиститьТрек(Команда)
	Для Каждого элем из элементы.ТрекНомера.ВыделенныеСтроки Цикл
		стрТрекНомер = Объект.ТрекНомера.НайтиПоИдентификатору(элем);
		стрТрекНомер.ТрекНомер = "";
	КонецЦикла;	
