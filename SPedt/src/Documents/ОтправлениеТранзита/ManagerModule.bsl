...
Функция ПолучитьТэг_groupsBill(Ссылка,НомерСтроки=Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	СУММА(ОтправлениеТранзитаТрекНомера.Длина * ОтправлениеТранзитаТрекНомера.Ширина *
		|		ОтправлениеТранзитаТрекНомера.Высота) КАК Объем,
	...
	Пока Выборка.Следующий() Цикл
		...
		СтоСП.Вставить_Тэг(ЗаписьXML, "weight", Выборка.вес);
		...
		ЗаписьXML.ЗаписатьНачалоЭлемента("deliveryMethods");
			...
			СтоСП.Вставить_Тэг(ЗаписьXML, "name", "Тариф ТК");
			СтоСП.Вставить_Тэг(ЗаписьXML, "description", "Почта России");
			СтоСП.Вставить_Тэг(ЗаписьXML, "type", "prepay");
			СтоСП.Вставить_Тэг(ЗаписьXML, "price", Выборка.СтоимостьДоставки);
			СтоСП.Вставить_Тэг(ЗаписьXML, "deliveryPeriod", "-");
