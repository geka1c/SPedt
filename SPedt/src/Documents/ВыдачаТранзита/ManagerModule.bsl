...
Функция Печать_ЧекПеремещениеЗаказов(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Макет 	= Документы.ВыдачаТранзита.ПолучитьМакет("ПФ_MXL_ЧекПеремещениеЗаказов");
...
Функция Печать_ЧекСИтогамиПоГабаритам(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Макет = Документы.ВыдачаТранзита.ПолучитьМакет("ПФ_MXL_ЧекСИтогамиПоГабаритам");
...
Функция ПечатьЭкспедиторскойРасписки(МассивОбъектов, ОбъектыПечати) Экспорт
	ТабДок					= Новый ТабличныйДокумент;
	Макет 					= Документы.ВыдачаТранзита.ПолучитьМакет("ПФ_MXL_ЭкспедиторскаяРасписка");
...
Функция ПечатьСжатогоЧека(МассивОбъектов, ОбъектыПечати) Экспорт
	ТабДок= Новый ТабличныйДокумент;
	Макет = Документы.ВыдачаТранзита.ПолучитьМакет("ПФ_MXL_ЧекСжатый");
...
Функция ПечатьНакладной(МассивОбъектов, ОбъектыПечати) Экспорт
	ТабДок= Новый ТабличныйДокумент;
	Макет = Документы.ВыдачаТранзита.ПолучитьМакет("ПФ_MXL_Накладная");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВыдачаТранзита.Дата,
...
Функция ПечатьНакладнойХабаровск(МассивОбъектов, ОбъектыПечати) Экспорт
	ТабДок= Новый ТабличныйДокумент;
	Макет = Документы.ВыдачаТранзита.ПолучитьМакет("ПФ_MXL_НакладнаяХабаровск");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВыдачаТранзита.Дата,
...
Функция ПечатьДоставкаКурьером(МассивОбъектов, ОбъектыПечати) Экспорт
	ТабДок= Новый ТабличныйДокумент;
	Макет = Документы.ВыдачаТранзита.ПолучитьМакет("ПФ_MXL_ПечатьДоставкаКурьером");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
...
Функция ПечатьПристроя(МассивОбъектов, ОбъектыПечати) Экспорт
	ТабДок= Новый ТабличныйДокумент;
	Макет = Документы.ВыдачаТранзита.ПолучитьМакет("ПФ_MXL_ПечатьПристроя");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВыдачаТранзита.Дата КАК Дата,
...
Функция ПечатьСпискаВыдачи(МассивОбъектов, ОбъектыПечати) Экспорт
	ТабДок= Новый ТабличныйДокумент;
	Макет = Документы.ВыдачаТранзита.ПолучитьМакет("ПФ_MXL_СписокВыдачи");
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
...
Функция ПолучитьКодЗаказа(Покупка,Участник)
	Если ЗначениеЗаполнено(Покупка.secureCode) Тогда
		...
		Возврат Лев(СтрЗаменить(Строка(х.ХешСумма)," ",""),4);
	Иначе 
		Возврат "Нет кода!";
	КонецЕсли;
КонецФункции
...
Функция ПолучитьТэг_transfer(Ссылка,НомерСтроки=Неопределено) Экспорт
	...
	| 				ИЛИ ВыдачаТранзитаПокупки.Ссылка.ТочкаТранзита = Значение(Справочник.ТочкиРаздачи.ПустаяСсылка) Тогда
	|			ТОГДА ВыдачаТранзитаПокупки.ТочкаНазначения.Код
