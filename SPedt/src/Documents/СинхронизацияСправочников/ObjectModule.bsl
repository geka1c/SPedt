...
Процедура ЗагрузитьСправочники() Экспорт
	ЗагрузитьПоДате();
	ЗагрузитьОтдельнымиЗапросами();
КонецПроцедуры

// Выполняет обмен с сайтом АПИ загрузка по дате
Процедура ЗагрузитьПоДате() Экспорт
...
// Выполняет обмен с сайтом отдельное АПИ для каждого раздела 
Процедура ЗагрузитьОтдельнымиЗапросами() Экспорт
	
	Для каждого элем из ПараметрыОтдельныеЗапросы Цикл
		Если элем.xmlName = "returnRights" Тогда
			НачалоД = ТекущаяДатаСеанса();
			Результат = СтоСПОбмен_Возвраты_СРазрешением.ОбновитьСписокПрав();
			Результат.вставить("НачалоД", НачалоД);			
			Результат.вставить("КонецД", ТекущаяДатаСеанса());
...
Функция Разбор(ПолученныеДанные)
	...
	Исключение
		...
		стр_Результат.Вставить("ОписаниеОшибки" ,ТекстОшибки);
	...
	Если Объект_distributors.dataByDates <>Неопределено Тогда
		...
		Если Объект_distributors.dataByDates[ИмяСписка] <> Неопределено Тогда
			ТЗрезультат=СтоСПОбмен_Общий.ТЗ_поСпискуXDTO(Объект_distributors.dataByDates[ИмяСписка][ИмяЭлемента]);
			Результат = СтоСПОбмен_ЗагрузкаСправочников.ОбработатьЗагруженныеСупергруппы(ТЗрезультат);
		...
		Если Объект_distributors.dataByDates[ИмяСписка] <> Неопределено Тогда
			ТЗрезультат=СтоСПОбмен_Общий.ТЗ_поСпискуXDTO(Объект_distributors.dataByDates[ИмяСписка][ИмяЭлемента]);
			Результат = СтоСПОбмен_ЗагрузкаСправочников.ОбработатьЗагруженныеПокупки(ТЗрезультат);
		...
		Если Объект_distributors.dataByDates[ИмяСписка] <> Неопределено Тогда
			ТЗрезультат=СтоСПОбмен_Общий.ТЗ_поСпискуXDTO(Объект_distributors.dataByDates[ИмяСписка][ИмяЭлемента]);
			Загуржено =СтоСПОбмен_ЗагрузкаСправочников.ОбработатьЗагруженныеКарты(ТЗрезультат);
		...
		Если Объект_distributors.dataByDates[ИмяСписка] <> Неопределено Тогда
			ТЗрезультат=СтоСПОбмен_Общий.ТЗ_поСпискуXDTO(Объект_distributors.dataByDates[ИмяСписка][ИмяЭлемента]);
			Загуржено = СтоСПОбмен_ЗагрузкаСправочников.ОбработатьЗагруженныхУчастников(ТЗрезультат);
			стр_Результат.Вставить(ИмяСписка,Загуржено);
		...
		Если Объект_distributors.dataByDates[ИмяСписка] <> Неопределено Тогда
			ТЗрезультат			= СтоСПОбмен_Общий.ТЗ_поСпискуXDTO(Объект_distributors.dataByDates[ИмяСписка][ИмяЭлемента]);
			Загружено 	= СтоСПОбмен_ЗагрузкаСправочников.ОбработатьЗагруженныеПосылки(ТЗрезультат);
	...
	Если Объект_distributors[ИмяСписка] <> Неопределено Тогда
		ТЗрезультат=СтоСПОбмен_Общий.ТЗ_поСпискуXDTO(Объект_distributors[ИмяСписка][ИмяЭлемента]);
		Загуржено = СтоСПОбмен_ЗагрузкаСправочников.ОбработатьЗагруженныеПунктыВыдачи(ТЗрезультат);
		стр_Результат.Вставить(ИмяСписка,Загуржено);
	КонецЕсли;
