...
// Заполнить заказами для обмена с сайтом
// 
// Параметры:
//  Документ - Неопределено - Документ
// 
// Возвращаемое значение:
//  Число - Количество выгружаемых заказов
Функция   ЗаполнитьНеОтправленными(Документ = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДокументыДляВыгрузки.Документ.Коробка.ШК КАК Мегаордер,
		...
		|	NULL Как ПартияЗаказа
		|ИЗ
		|	ДокументыДляВыгрузки КАК ДокументыДляВыгрузки
		|ГДЕ
		|	ТипЗначения(ДокументыДляВыгрузки.Документ) = Тип(Документ.ОтправлениеТранзита)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ДокументыДляВыгрузки.Документ.Дата,
		|	ДокументыДляВыгрузки.Документ.Номер,
		|	ВыдачаТранзитаПокупки.Партия
		|ИЗ
		|	ДокументыДляВыгрузки КАК ДокументыДляВыгрузки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВыдачаТранзита.Покупки КАК ВыдачаТранзитаПокупки
		|		ПО ДокументыДляВыгрузки.Документ = ВыдачаТранзитаПокупки.Ссылка
		|		И (ДокументыДляВыгрузки.типОбмена = ЗНАЧЕНИЕ(Перечисление.ТипыОбменов100сп.Транзит))
		|		И (ВыдачаТранзитаПокупки.ШК.типМегаордера <> ЗНАЧЕНИЕ(Перечисление.типМегаордера.Супергруппа))
		|		И (ВыдачаТранзитаПокупки.ШК <> ЗНАЧЕНИЕ(Справочник.Мегаордера.нулевой))
		|Где 	 ТипЗначения(ВыдачаТранзитаПокупки.Партия) <> Тип(Документ.ЗаказыДляТранзита)
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВыдачаТранзитаПокупки.ШК,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВыдачаТранзита.Покупки КАК ВыдачаТранзитаПокупки
		|		ПО ДокументыДляВыгрузки.Документ = ВыдачаТранзитаПокупки.Ссылка
		|		И (ДокументыДляВыгрузки.типОбмена = ЗНАЧЕНИЕ(Перечисление.ТипыОбменов100сп.ЗаказыДляТранзита))
		|Где 	 ТипЗначения(ВыдачаТранзитаПокупки.Партия) = Тип(Документ.ЗаказыДляТранзита)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Заказы.Посылка.ШК Как Мегаордер,
		|	Заказы.КодЗаказа Как КодЗаказа,
		|	ДокументыДляВыгрузки.Документ.Ссылка Как Партия,
		|	Заказы.Ссылка как ОтправлениеТранзита,
		|	ВыдачаТранзитаПокупки.НомерСтроки Как СтрокаВПартии,
		|	ДокументыДляВыгрузки.типОбмена как ТипОбмена,
		|	ДокументыДляВыгрузки.Документ.Дата как Период,
		|	ДокументыДляВыгрузки.Документ.Номер как ПартияНомер,
		|	ЗаказыТрекНомера.ТрекНомер,
		|	ЗаказыТрекНомера.Отправление,
		|	ЗаказыТрекНомера.Длина,
		|	ЗаказыТрекНомера.Ширина,
		|	ЗаказыТрекНомера.Высота,
		|	ЗаказыТрекНомера.Вес,
		|	ЗаказыТрекНомера.Тариф,
		|	ЗаказыТрекНомера.СрокДоставки,
		|	ЗаказыТрекНомера.СтоимостьДоставки
		|ИЗ
		|	ДокументыДляВыгрузки КАК ДокументыДляВыгрузки
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ВыдачаТранзита.Покупки КАК ВыдачаТранзитаПокупки
		|		ПО ДокументыДляВыгрузки.Документ = ВыдачаТранзитаПокупки.Ссылка
		|		И (ДокументыДляВыгрузки.типОбмена = ЗНАЧЕНИЕ(Перечисление.ТипыОбменов100сп.Транзит))
		|		И (ВыдачаТранзитаПокупки.ШК = ЗНАЧЕНИЕ(Справочник.Мегаордера.нулевой))
		|		И (ТипЗначения(ВыдачаТранзитаПокупки.Партия) = Тип(Документ.ОтправлениеТранзита))
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ЗаказыТранскортныхКомпаний КАК ЗаказыТрекНомера
		|		ПО ВыдачаТранзитаПокупки.Партия = ЗаказыТрекНомера.Регистратор
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ОтправлениеТранзита.Заказы КАК Заказы
		|		ПО Заказы.Ссылка = ЗаказыТрекНомера.Регистратор
		|		и Заказы.Отправление = ЗаказыТрекНомера.Отправление";
	
	Если  Документ <> Неопределено Тогда
		Запрос.Текст =СтрЗаменить(Запрос.Текст,"И Истина"," и Обмен100СПрнОстатки.Партия = &Документ"); 
	КонецЕсли;	
	Запрос.УстановитьПараметр("Документ", Документ);
	Запрос.УстановитьПараметр("НаДату", КонецДня(Дата));
	
	МассивРезультатов = Запрос.ВыполнитьПакет();
	ВыгружаемыДокументы.Очистить();
	ВыгружаемыеДанные.Очистить();
	ВыгружаемыеДанныеТК.Очистить();
	
	
	
	ВыборкаДокументы = МассивРезультатов[1].Выбрать();
	Пока ВыборкаДокументы.Следующий() Цикл
		новаяСтрока = ВыгружаемыДокументы.Добавить();
		ЗаполнитьЗначенияСвойств(новаяСтрока,ВыборкаДокументы);
	КонецЦикла;	
	
	ВыборкаДанные = МассивРезультатов[2].ВЫбрать();
	Пока ВыборкаДанные.Следующий() Цикл
		новаяСтрока = ВыгружаемыеДанные.Добавить();
		ЗаполнитьЗначенияСвойств(новаяСтрока,ВыборкаДанные);
	КонецЦикла;	
	
	ВыборкаДанныеТК = МассивРезультатов[3].ВЫбрать();
	Пока ВыборкаДанныеТК.Следующий() Цикл
		новаяСтрока = ВыгружаемыеДанныеТК.Добавить();
		ЗаполнитьЗначенияСвойств(новаяСтрока,ВыборкаДанныеТК);
	КонецЦикла;	
...
// Формирует пакет данных для выгрузки на сайт
// 
// Возвращаемое значение:
//  Строка - Строка Хмл для выгрузки на сайт
Функция   СкомпоноватьДляВыгрузки() Экспорт
	...
	ЗаписьXML.ЗаписатьАтрибут("apiVersion","19");	

	///////
	ЗаписьXML.ЗаписатьНачалоЭлемента("groupsBill");	
	ТипОбмена = Перечисления.ТипыОбменов100сп.ГруппыДоставкиЗапросОплаты;
	...
	Для каждого Элем Из тзТипыПартии Цикл
		тзПартии_groupsBill	= тзВсеДокументы.Скопировать(новый Структура("ТипПартии,типОбмена",Элем.ТипПартии,ТипОбмена),"Партия");
		хмл_groupsBill			= Документы[Метаданные.НайтиПоТипу(Элем.ТипПартии).Имя].ПолучитьТэг_groupsBill(тзПартии_groupsBill.ВыгрузитьКолонку("Партия"));	
		ЗаписьXML.ЗаписатьБезОбработки(хмл_groupsBill);
	КонецЦикла;	
	ЗаписьXML.ЗаписатьКонецЭлемента();           //groupsBill
	//////
	...
	///////
	ЗаписьXML.ЗаписатьНачалоЭлемента("preTransfers");	
	ТипОбмена = Перечисления.ТипыОбменов100сп.ЗаказыДляТранзита;
	тзТипыПартии=тзВсеДокументы.Скопировать(новый Структура("ТипОбмена",ТипОбмена),"ТипПартии");
	тзТипыПартии.Свернуть("ТипПартии");
	Для каждого Элем Из тзТипыПартии Цикл
		тзПартии_incomes	= тзВсеДокументы.Скопировать(новый Структура("ТипПартии,типОбмена",Элем.ТипПартии,ТипОбмена),"Партия");
		хмл_income			= Документы[Метаданные.НайтиПоТипу(Элем.ТипПартии).Имя].ПолучитьТэг_preTransfer(тзПартии_incomes.ВыгрузитьКолонку("Партия"));	
		ЗаписьXML.ЗаписатьБезОбработки(хмл_income);
	КонецЦикла;		
	ЗаписьXML.ЗаписатьКонецЭлемента();           //preTransfers
	///////
	...
	ЗаписьXMLтк=новый ЗаписьXML;
	ЗаписьXMLтк.УстановитьСтроку("UTF-8");	
	ЗаписьXMLтк.ЗаписатьНачалоЭлемента("Служебный");
	Для каждого элем из ВыгружаемыеДанныеТК Цикл
		Если элем.ОтправлениеТранзита.ТочкаНазначения.Код = "0201" или
			 элем.ОтправлениеТранзита.ТочкаНазначения.Код = "0020" или
			 элем.ОтправлениеТранзита.ТочкаНазначения.Код = "0021"  Тогда
			тк = "russianPost";
		Иначе
			тк = "sdec";	
		КонецЕсли;		
		
		ЗаписьXMLтк.ЗаписатьНачалоЭлемента("transfer");
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"distributorCode"	, 	элем.ОтправлениеТранзита.ТочкаНазначения.Код);
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"groupCode"	, 	"");
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"paidSum"	, 0);
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"date"	, 	Элем.Партия.Дата);
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"arrivalNumber"	, 	Прав(Элем.ОтправлениеТранзита.Номер,6));
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"orderType", 	"package");
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"packageId"	, 	Формат(Элем.Мегаордер.Посылка.Код,"ЧГ=0"));
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"trackingNumber"	, 	Элем.ТрекНомер);
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"transportCompany"	, 	тк);
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"price"	, 	Формат(Элем.СтоимостьДоставки,"ЧГ=0"));
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"weight"	, 	Формат(Элем.Вес,"ЧГ=0"));
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"width"	, 	Формат(Элем.Длина,"ЧГ=0"));
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"height"	, 	Формат(Элем.Ширина,"ЧГ=0"));
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"length"	, 	Формат(Элем.Высота,"ЧГ=0"));
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"name"	, 	Элем.Тариф);
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"description"	, 	"");
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"deliveryPeriod"	, 	Элем.СрокДоставки);
		
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"uid"	, 			Формат(Число(элем.Мегаордер.Покупка.Участник.Код),"ЧГ=0"));
		
		ЗаписьXMLтк.ЗаписатьКонецЭлемента();	
	
	КонецЦикла;
    ЗаписьXMLтк.ЗаписатьКонецЭлемента();
	рез=ЗаписьXMLтк.Закрыть();
	рез=СтрЗаменить(рез,"<Служебный>","");
	рез=СтрЗаменить(рез,"</Служебный>","");	
	ЗаписьXML.ЗаписатьБезОбработки(рез);
...
Функция   ВыгрузитьНаСайт() Экспорт
	...
	Если стр_Ответа.Разбор.Свойство("preTransfers",тз) Тогда
		preTransfers_ОтразитьРезультатВыгрузки(тз);
	КонецЕсли;	
	Если стр_Ответа.Разбор.Свойство("returns",тз) Тогда
		returns_ОтразитьРезультатВыгрузки(тз);
	КонецЕсли;	
	
	Если стр_Ответа.Разбор.Свойство("groupsBill",тз) Тогда
		groupsBill_ОтразитьРезультатВыгрузки(тз);
	КонецЕсли;		
...
//	Если ВыгружаемыеДанныеТК.Количество()>0 Тогда 
//		Для каждого элем из тз Цикл	
//			Если ЗначениеЗаполнено(элем.packageId) Тогда
//				найдено = ВыгружаемыеДанныеТК.НайтиСтроки(новый Структура("КодЗаказа",элем.packageId));
//				Для Каждого стр из найдено Цикл
//					стр.Отправлено = Истина;
//					стр.РезультатОбмена	= элем.result;
//					Если элем.result="error" Тогда
//						стр.СообщениеОшибки =элем.message;
//					КонецЕсли;			
//				КонецЦикла;
//			КонецЕсли;	
//		КонецЦикла;
//	КонецЕсли;	
	...
	|ПОМЕСТИТЬ ПакетТК
	|ИЗ
	|	Документ.Выгрузка_100СП.ВыгружаемыеДанныеТК КАК Выгрузка_100СПВыгружаемыеДанные
	|ГДЕ
	|	Выгрузка_100СПВыгружаемыеДанные.Ссылка = &Ссылка
	|	И Выгрузка_100СПВыгружаемыеДанные.типОбмена = ЗНАЧЕНИЕ(Перечисление.ТипыОбменов100сп.Транзит)
	|;
	...
	|ИЗ
	|	ВТ КАК ВТ
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПакетТК КАК ПакетНаСайтДетально
	|		ПО (ВЫБОР
	|				КОГДА (ВЫРАЗИТЬ(ВТ.orderType КАК СТРОКА(2))) = ""sp""
	|					ТОГДА ВТ.pid = ПакетНаСайтДетально.Мегаордер.Покупка.Код
	|							И ТИПЗНАЧЕНИЯ(ПакетНаСайтДетально.Мегаордер.Покупка) = ТИП(Справочник.Покупки)
	...
	ТЗ = Результаты[3].Выгрузить();
	ТЗ_ТК = Результаты[4].Выгрузить();
	
	//Отражаем результат обмена
	Если тз<>неопределено и ВыгружаемыеДанные.Количество()>0 Тогда  
		Для каждого стр из тз Цикл
			Если стр.НомерСтроки =null Тогда Продолжить; КонецЕсли;
			Если Стр.arrivalNumber<>Прав(Стр.Партия.Номер,(СтрДлина(Стр.Партия.Номер)-3)) Тогда Продолжить; КонецЕсли;
	...
	Если ТЗ_ТК<>неопределено и ВыгружаемыеДанныеТК.Количество()>0 Тогда  
		Для каждого элем из ВыгружаемыеДанныеТК Цикл
			Если ЗначениеЗаполнено(элем.Мегаордер) Тогда
				найдено = ТЗ_ТК.НайтиСтроки(новый Структура("Мегаордер",элем.Мегаордер));
				Если найдено.Количество() = 0 Тогда
					Элем.Отправлено = Истина;
					Элем.РезультатОбмена	= "error";
					Элем.СообщениеОшибки = "Не найдено в ответе";
				Иначе	
					Элем.Отправлено = Истина;
					Элем.РезультатОбмена	= найдено[0].result;
					Если Элем.РезультатОбмена="error" Тогда
						Элем.СообщениеОшибки =найдено[0].message;
					КонецЕсли;			
				КонецЕсли
			Иначе
					Элем.Отправлено = Истина;
					Элем.РезультатОбмена	= "error";
					Элем.СообщениеОшибки = "Не заполнен ШК отправления";
			КонецЕсли;				
		КонецЦикла;
	КонецЕсли;			
...
Процедура preTransfers_ОтразитьРезультатВыгрузки(тз) 
	массСтрок=тз.НайтиСтроки(новый структура("orderType","group"));
	Для каждого стр из массСтрок Цикл
		стр.groupCode=Формат(число(стр.groupCode),"ЧЦ=8; ЧН=0; ЧВН=; ЧГ=0"); 
	КонецЦикла;		
	...
	|ГДЕ
	|	Выгрузка_100СПВыгружаемыеДанные.Ссылка = &Ссылка
	|	И Выгрузка_100СПВыгружаемыеДанные.типОбмена = ЗНАЧЕНИЕ(Перечисление.ТипыОбменов100сп.ЗаказыДляТранзита)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		КОНЕЦ)
	|		И (ВТ.uid = ПакетНаСайтДетально.Мегаордер.Участник.Код
	|		или ТИПЗНАЧЕНИЯ(ПакетНаСайтДетально.Мегаордер.Покупка) = ТИП(Справочник.Посылки))";
	Запрос.Параметры.Вставить("ТЗ",ТЗ);
	Запрос.Параметры.Вставить("Ссылка",Ссылка);
	Результат=Запрос.Выполнить();
	ТЗ = Результат.Выгрузить();
...
Процедура groupsBill_ОтразитьРезультатВыгрузки(тз)
	найдено = ВыгружаемыеДанные.НайтиСтроки(Новый Структура("ТипОбмена",Перечисления.ТипыОбменов100сп.ГруппыДоставкиЗапросОплаты));
	ДЛя каждого элем из найдено Цикл
		КодГруппы =Формат(Число(СтрЗаменить(Элем.Мегаордер.Покупка.Код,"гд_","")),"ЧГ=0");
		НайденоВОтвете =  тз.НайтиСтроки(Новый Структура("delivery_group_id",КодГруппы));
		
		Если НайденоВОтвете.Количество() =0 Тогда
			Элем.СообщениеОшибки = "Не найдено в ответет";
		Иначе
			элемОтвет = НайденоВОтвете[0];
			элем.Отправлено=Истина;
			элем.РезультатОбмена=элемОтвет.result;

			Если элемОтвет.result="error" Тогда
				Элем.СообщениеОшибки =элемОтвет.message;
				Продолжить;
			Конецесли;
		КонецЕсли;	
	КонецЦикла;
