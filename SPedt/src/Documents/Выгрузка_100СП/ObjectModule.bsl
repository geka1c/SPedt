...
Функция   СкомпоноватьДляВыгрузки() Экспорт
	...
	Для каждого элем из ВыгружаемыеДанныеТК Цикл
		...
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"orderType", 	"sp");
		...
		СтоСП.Вставить_Тэг(ЗаписьXMLтк,"orderId"	, 	Элем.КодЗаказа);
...
Процедура transfers_ОтразитьРезультатВыгрузки(тз) 
	...
	Если ВыгружаемыеДанныеТК.Количество()>0 Тогда 
		Для каждого элем из тз Цикл	
			Если ЗначениеЗаполнено(элем.orderId) Тогда
				найдено = ВыгружаемыеДанныеТК.НайтиСтроки(новый Структура("КодЗаказа",элем.orderId));
