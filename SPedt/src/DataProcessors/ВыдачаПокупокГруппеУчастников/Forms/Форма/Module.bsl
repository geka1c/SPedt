...
Процедура ВыдатьПокупкиНаСервере(Списывать)
	...
	Для каждого элем из отмеченныеУчастники Цикл
		...
		обРасходная.ЗаполнитьОстатками(,Истина);
		Если обРасходная.Покупки.Количество()>0 Тогда
...
Процедура ПросмотрНаСервере()
	...
	Для каждого элем из тз Цикл
		Если не Элем.Ссылка.ПометкаУдаления и
			 	Элем.Ссылка.Покупки.Количество()>0 Тогда
			массОбъектов.Добавить(Элем.ссылка);
		КонецЕсли;
	КонецЦикла;
