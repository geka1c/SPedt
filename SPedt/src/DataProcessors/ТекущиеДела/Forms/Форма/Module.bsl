...
&НаКлиенте
Процедура ОбменДанныим(Команда)
	ОбработчикРезультата = Новый ОписаниеОповещения("ПрименитьНастройкиПанелиДел", ЭтотОбъект);
	ОткрытьФорму("Обработка.ЗагрузкаССайта.Форма.Форма", ,,,,,ОбработчикРезультата);
КонецПроцедуры



&НаКлиенте
Процедура ОтложитьВыгрузку(Команда)
	ОтложитьВыгрузкуНаСервере();
	ЗапуститьОбновлениеСпискаТекущихДел();
КонецПроцедуры

&НаСервере
Процедура ОтложитьВыгрузкуНаСервере()
	текДела = СтоСП_ТекущиеДела.Получитьданные();
	ОтсрочкаВыгрузки = Константы.ОтсрочкаВыгрузки.Получить();
	
	НоваяДата = текДела.ДатаВыгрузкиСледующая + 30*60 -1;	
	Константы.ОтсрочкаВыгрузки.Установить(НоваяДата);
