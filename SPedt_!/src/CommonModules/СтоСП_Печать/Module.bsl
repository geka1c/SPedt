...
Функция ТабДок_СупперГруппа(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Для каждого Выборка из МассивОбъектов Цикл
		...
		Для каждого ВыборкаПокупки из выборка.Покупки Цикл   
			...
			Если  ТипЗнч(ВыборкаПокупки.Партия) = Тип("ДокументСсылка.ОтправлениеТранзита") и
					 ВыборкаПокупки.Партия.ТрекНомера.Количество() > 0 Тогда
					 	
				Для Каждого отпр Из ВыборкаПокупки.Партия.ТрекНомера Цикл
					ОбластьПокупки.Параметры.Заполнить(ВыборкаПокупки);
					ОбластьПокупки.Параметры.Участник 		= ВыборкаПокупки.Партия.Участник;
					ОбластьПокупки.Параметры.КодПокупки 	= отпр.ТрекНомер;
					ОбластьПокупки.Параметры.ТипПокупки 	= ТипЗнч(ВыборкаПокупки.Покупка);
					ОбластьПокупки.Параметры.КодУчастника 	= ВыборкаПокупки.Партия.Участник.Код;
			...
			Если  ТипЗнч(ВыборкаПокупки.Покупка) = Тип("СправочникСсылка.Коробки")  и
				...
				Если КодПокупки ="" Тогда Продолжить; КонецЕсли;
				Участник = "";
			Иначе
