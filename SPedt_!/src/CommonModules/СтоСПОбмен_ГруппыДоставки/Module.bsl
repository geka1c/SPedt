...
Функция ПолучитьТоблицуГруппы() 
	...
	ТЗ.КОлонки.Добавить("ВерсияТарифов" ,			новый ОписаниеТипов("Число"));
...
Функция СозданиеГруппПоДаннымСайта(СписокXDTO,СвойстваДаты=неопределено) Экспорт
	...
	///	стр_тз.Группа 					= СП_РаботаСоСправочниками.ПолучитьКоробкуПо_Коду(стр_тз.КодКоробки);
		...
		Если элем.Свойства().Получить("priceCalculationVersion") = Неопределено Тогда
			стр_тз.ВерсияТарифов = 0;
		Иначе	
			стр_тз.ВерсияТарифов			= ?(элем.priceCalculationVersion = "", 0, Число(элем.priceCalculationVersion));
		КонецЕсли;	
...
// Формирование новых элементов справочников	
	...
	|	тзФ.ВерсияТарифов КАК ВерсияТарифов,
	...
	|	Ф.ВерсияТарифов КАК ВерсияТарифов,
	...
	|	ГруппыИтог.ВерсияТарифов КАК ВерсияТарифов,
	...
	|		ИЛИ ГруппыИтог.Группа.ВерсияТарифов <> ГруппыИтог.ВерсияТарифов
	...
	|	ГруппыИтог2.ВерсияТарифов КАК ВерсияТарифов,
	...
	|			СоставИзменился КАК СоставИзменился)"; 
		...
		Пока ВыборкаГруппы.Следующий() Цикл
				...
				Если ВыборкаГруппы.ШапкаИзменилась Тогда
					...
					параметрыКоробки.Вставить("ВерсияТарифов", 			ВыборкаГруппы.ВерсияТарифов);
