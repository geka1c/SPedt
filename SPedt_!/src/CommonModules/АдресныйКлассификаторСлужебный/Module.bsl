...
Функция РаспознатьАдресВебСервис(ЧастиАдреса, Представление)
	...
	ТекстGETЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("autocomplete?text=%1&limit=20",
	...
	Для каждого Данные Из РезультатВебСервис.Данные Цикл
		...
		НайденныйАдрес = ОпределитьСоответствиеАдреса(Данные, Текст, Истина);
		Если НайденныйАдрес = Неопределено Тогда
			НайденныйАдрес = ОпределитьСоответствиеАдреса(Данные, Текст, Ложь);
		КонецЕсли;
		
		Если НайденныйАдрес <> Неопределено Тогда
			НайденныйАдрес.apartments = Помещения;
...
Функция ОпределитьСоответствиеАдреса(Знач Данные, Знач Текст, Муниципальный)
	
	ПредставлениеАдреса = ?(Муниципальный, Данные.munValue, Данные.value);
	
	ПозицияИндекса = СтрНайти(ПредставлениеАдреса, "(");
	Если ПозицияИндекса > 0 Тогда
		Представление = СокрЛП(Лев(ПредставлениеАдреса, ПозицияИндекса - 1));
		Если СтрСравнить(Текст, Представление) = 0 Тогда
			Возврат АдресДляСохраненияВБазе(Данные, Муниципальный);
		КонецЕсли;
	КонецЕсли;
