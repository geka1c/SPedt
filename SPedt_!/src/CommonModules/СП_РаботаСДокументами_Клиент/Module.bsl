...
Процедура ОткрытьФорму_Расходная(ПараметрыКарты)  Экспорт
	...
	ОткрытьФорму("Документ.Расходная.ФормаОбъекта", ПараметрыКарты, ,Истина , , , , );
...
Процедура ОткрытьФорму_Приходная(ПараметрыЗаказа)  Экспорт
	Если   аСППрверкиКлиент.НаличиеОткрытыхОкон("Приходная") Тогда
    	ПоказатьПредупреждение(,"Существует не завершенная операция Поступления.Закройте все окна Прихода и повторите попытку!!!",30,"Предупреждение");
		Возврат;		
	КонецЕсли;
...
Процедура ОткрытьФорму_РазборКоробки(ПараметрыКоробки)  Экспорт
	Если   аСППрверкиКлиент.НаличиеОткрытыхОкон("Разбор") Тогда
    	ПоказатьПредупреждение(,"Существует не завершенная операция Разбора коробки .Закройте и повторите попытку!!!",30,"Предупреждение");
		Возврат;		
	КонецЕсли;
...
Процедура ОткрытьФорму_Продажа(Параметры)  Экспорт
	Если   аСППрверкиКлиент.НаличиеОткрытыхОкон("Продажа") Тогда
    	ПоказатьПредупреждение(,"Существует не завершенная операция Продажи. Закройте и повторите попытку!!!",30,"Предупреждение");
		Возврат;		
	КонецЕсли;
