Функция Привязать(Объект) Экспорт
	Коробка = Объект.Коробка;
	ОборотнаяТара = Объект.ОборотнаяТара;
	
	
	хмл_отправили						= СтоСПОбмен_Запрос.ОборотнаяТараПривязать(Коробка, ОборотнаяТара);
	
	СтрокаПротокола						= СтоСПОбмен_Выгрузка100сп.СтруктураПротокола();
	СтрокаПротокола.ДатаНачала			= ТекущаяДата();
	СтрокаПротокола.Описание			= "Привязать оборотную тару к группе";
	...
	Если не стр_Ответа.Свойство("Разбор") или стр_Ответа.Разбор.reusableStickerConnect.Количество() = 0 Тогда
		СтрокаПротокола.СообщениеОшибки = "Не удалось соединиться.";
		СтрокаПротокола.Результат	=	Ложь;
	Иначе	
		Если стр_Ответа.Разбор.reusableStickerConnect[0].result = "ok" или
			стр_Ответа.Разбор.reusableStickerConnect[0].result = "error" и
			стр_Ответа.Разбор.reusableStickerConnect[0].message = "Стикеры уже связаны между собой" Тогда
			СтрокаПротокола.Результат	=	Истина;
		Иначе	
			СтрокаПротокола.СообщениеОшибки = стр_Ответа.Разбор.reusableStickerConnect[0].message;
			СтрокаПротокола.Результат	=	Ложь;
		КонецЕсли;	
	КонецЕсли;
