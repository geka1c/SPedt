...
Процедура СписокСчетаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элементы.СписокСчета.ТекущиеДанные<>Неопределено Тогда
		ПоказатьЗначение(,Элементы.СписокСчета.ТекущиеДанные.Ссылка);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьКалендарь()
	...
	Для каждого СтрокаДанных Из ТаблицаСтатусов Цикл
		...
		Если НайденнаяСтрока = Неопределено Тогда
			...
			Если  СтрокаДанных.Статус="Ожидает доставки" Тогда
		...
		НайденнаяСтрока.СтрокаДокументов = НайденнаяСтрока.СтрокаДокументов + СтрокаДанных.Статус +" - " + СтрокаДанных.Груз +"м,  " + СтрокаДанных.ГрузШт +"шт.";
	КонецЦикла;
	...
	Для Строка = 2 По 6 Цикл                            
		...
		ВсегоОбластей=тзОбластиДня.Количество()+2;
...
Функция ПолучитьСтатусыЗаПериод(НачалоПериода,КонецПериода)
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(СтатусыДоставки.Период, ДЕНЬ) КАК Период,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СтатусыДоставки.Груз) КАК Груз,
	|	СтатусыДоставки.Статус,
	|	СУММА(СтатусыДоставки.Груз.Количество) КАК ГрузШт,
	|	МАКСИМУМ(ВЫБОР
	|				КОГДА ТИПЗНАЧЕНИЯ(СтатусыДоставки.Регистратор) = ТИП(Документ.СчетКонтрагента)
	|						И (СтатусыДоставки.Регистратор.НомерОтправкиГруза = """"
	|							ИЛИ СтатусыДоставки.Регистратор.Сумма = 0)
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ
	|			ИЛИ ВЫБОР
	|				КОГДА СтатусыДоставки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправкиГруза.ВыехалСТочки)
	|					ТОГДА СтатусыДоставкиСчетТК.Статус ЕСТЬ NULL 
	|							ИЛИ СтатусыДоставкиСчетЭК.Статус ЕСТЬ NULL 
	|				ИНАЧЕ ЛОЖЬ
	|			КОНЕЦ) КАК ЕстьОшибки
	|ПОМЕСТИТЬ втСтатусы
	|ИЗ
	|	РегистрСведений.СтатусыДоставки КАК СтатусыДоставки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыДоставки.СрезПоследних(, Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправкиГруза.ДобавленСчетЭК)) КАК СтатусыДоставкиСчетЭК
	|		ПО СтатусыДоставки.Груз = СтатусыДоставкиСчетЭК.Груз
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыДоставки.СрезПоследних(, Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправкиГруза.ВТранспортнойКомпании)) КАК СтатусыДоставкиСчетТК
	|		ПО СтатусыДоставки.Груз = СтатусыДоставкиСчетТК.Груз
	|ГДЕ
	|	СтатусыДоставки.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(СтатусыДоставки.Период, ДЕНЬ),
	|	СтатусыДоставки.Статус
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(СтатусыДоставки.ОриентеровачнаяДатаДоставки, ДЕНЬ),
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СтатусыДоставки.Груз),
	|	""Ожидает доставки"",
	|	СУММА(СтатусыДоставки.Груз.Количество),
	|	ЛОЖЬ
	|ИЗ
	|	РегистрСведений.СтатусыДоставки КАК СтатусыДоставки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыДоставки.СрезПоследних(, ) КАК СтатусыДоставкиСрезПоследних
	|		ПО СтатусыДоставки.Груз = СтатусыДоставкиСрезПоследних.Груз
	|			И (СтатусыДоставкиСрезПоследних.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправкиГруза.Доставлен)
	|				ИЛИ СтатусыДоставкиСрезПоследних.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправкиГруза.Утрачен))
	|ГДЕ
	|	СтатусыДоставки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправкиГруза.ВТранспортнойКомпании)
	|	И СтатусыДоставки.ОриентеровачнаяДатаДоставки МЕЖДУ &НачалоПериода И &КонецПериода
	|	И СтатусыДоставкиСрезПоследних.Груз ЕСТЬ NULL 
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(СтатусыДоставки.ОриентеровачнаяДатаДоставки, ДЕНЬ)
	|
	...
	|
	|ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(ПоступленияОтПоставщика.Дата, ДЕНЬ),
	|	СУММА(ПоступленияОтПоставщика.КоличествоМест),
	|	""От поставщика"",
	|	СУММА(ПоступленияОтПоставщика.Количество),
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ПоступленияОтПоставщика.КоличествоМест = 0
	|					ИЛИ ПоступленияОтПоставщика.Количество = 0
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ)
	|ИЗ
	|	Документ.ПоступленияОтПоставщика КАК ПоступленияОтПоставщика
	|ГДЕ
	|	ПоступленияОтПоставщика.Проведен
	|	И ПоступленияОтПоставщика.Дата МЕЖДУ &НачалоПериода И &КонецПериода
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(ПоступленияОтПоставщика.Дата, ДЕНЬ)
	|;
	...
	|ВЫБРАТЬ
	|	втСтатусы.Период,
	|	СУММА(втСтатусы.Груз) КАК Груз,
	|	втСтатусы.Статус,
	|	СУММА(втСтатусы.ГрузШт) КАК ГрузШт,
	|	МАКСИМУМ(втСтатусы.ЕстьОшибки) КАК НетСчета
	|ИЗ
...
Процедура СписокЗаявкиОбщийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Элементы.СписокЗаявкиОбщий.ТекущиеДанные.ЗаявкаВТК) Тогда
		ПоказатьЗначение(,Элементы.СписокЗаявкиОбщий.ТекущиеДанные.ЗаявкаВТК);
	КонецЕсли;
КонецПроцедуры
