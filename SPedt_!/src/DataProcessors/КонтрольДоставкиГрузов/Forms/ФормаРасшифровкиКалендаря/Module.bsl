...
//	ДобавитьВКорзинуНаСервере();
	...
	ОповеститьОВыборе(ПолучитьГСпоРегистратору(ТекущийСписок.ТекущиеДанные.Регистратор));
КонецПроцедуры
...
Процедура РасшифроватьГруз()
	...
	Если ТекущийСписок.ТекущиеДанные<>Неопределено Тогда
		Если ТекущийСписок.ТекущиеДанные.Свойство("Регистратор") Тогда
			Регистратор=ТекущийСписок.ТекущиеДанные.Регистратор;
	...
	Иначе
		Регистратор=Неопределено;
	КонецЕсли;	
	РасшифроватьГрузНаСервере(Регистратор);
КонецПроцедуры	

Процедура РасшифроватьГрузНаСервере(Регистратор)
	
	
	
	РасшифровкаГруза.Параметры.УстановитьЗначениеПараметра("Ссылка",ПолучитьГСпоРегистратору(Регистратор));	
	//СписокДокументов.Параметры.УстановитьЗначениеПараметра("Груз",Регистратор);
КонецПроцедуры	


Функция ПолучитьГСпоРегистратору(Регистратор)
	Запрос=новый Запрос;
	Запрос.Текст=
	"ВЫБРАТЬ
	|	ПриходнаяПокупки.Покупка КАК ГС
	|ПОМЕСТИТЬ Сбор
	|ИЗ
	...
	|ГДЕ
	|	ЗаявкаВТранспортнуюКомпаниюЗаказы.Ссылка = &Ссылка
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ОтчетОДоставкеЗаказы.Груз
	|ИЗ
	...
	Возврат Результат.Выгрузить().ВыгрузитьКолонку("ГС");
