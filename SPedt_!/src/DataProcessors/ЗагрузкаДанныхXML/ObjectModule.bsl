...
&НаСервере
Процедура ИнициализацияУпр() Экспорт
	...
	ИспользующиеИтоги = Новый Массив;
	//Корень = ДеревоМетаданных.Строки.Добавить();
	Корень = ДеревоМетаданных.Добавить();
	ПостроениеПоддереваОбъекта(Метаданные, Корень, МетаданныеОписание);
	//СверткаПоддереваОбъекта(Корень);
...
Процедура ВыполнитьЗагрузку(ИмяФайла) Экспорт
	...
	СообщитьУпр("Начало загрузки:   " + ТекущаяДата());
	...
	Пока ВозможностьЧтенияXML(ЧтениеXML) Цикл
		...
		Исключение
			...
			Иначе
				...
				Попытка
					СообщитьУпр("Ошибка при загрузке данных. Объект: " + ЗаписанноеЗначение + ", Тип: " + ТипЗнч(ЗаписанноеЗначение) + "
						|Ошибка : " + ТекстОшибки, СтатусСообщения.Важное);
				Исключение
					СообщитьУпр("Ошибка при загрузке данных.
						|Ошибка : " + ТекстОшибки, СтатусСообщения.Важное);
				КонецПопытки;	
	...
	СообщитьУпр("Загружено объектов: " + Загружено);
	СообщитьУпр("Окончание загрузки: " + ТекущаяДата());
	#Если Клиент Тогда
...
Процедура ВыполнитьПроверкуДляВыгрузкиОбъектовИзПланаОбмена(СсылкаНаУзел) Экспорт
	...
	#Если Клиент Тогда
	СообщитьУпр("Отобразить результат");
...
Процедура ВыполнитьВыгрузку(ИмяФайла, ТолькоПроверкаНедопустимыхСимволов = Ложь) Экспорт
	...
	Иначе
		...
		Если НЕ ТолькоПроверкаНедопустимыхСимволов Тогда
			СообщитьУпр("Начало выгрузки:   " + ТекущаяДата());
		Иначе
			СообщитьУпр("Начало проверки:   " + ТекущаяДата());
		КонецЕсли;
	...
	Если НЕ ТолькоПроверкаНедопустимыхСимволов Тогда
		СообщитьУпр("Выгружено объектов: " + ВсегоОбработанныхЗаписей());
        СообщитьУпр("Окончание выгрузки: " + ТекущаяДата());
	#Если Клиент Тогда
		Предупреждение("Выгрузка данных успешно завершена", 10);
	#КонецЕсли
	Иначе		
	#Если Клиент Тогда
		СообщитьУпр("Проверено объектов: " + ВсегоОбработанныхЗаписей());
		СообщитьУпр("Окончание проверки: " + ТекущаяДата());		
		ОтобразитьОбъектыСОшибкамиВыгрузки(ОбъектыВыгруженныеСОшибками);		
	#КонецЕсли
	КонецЕсли;
		...
		Если ЭтоЗапросПоОбъекту Тогда
			...
			Исключение
				...
				Иначе
					...
					СообщитьУпр(ИтоговаяСтрокаСообщения);
...
Процедура УстановитьИспользованиеИтогов()
	...
	Если ИспользоватьИтоги ИЛИ
		 ИспользующиеИтоги=Неопределено Тогда //VVR
		Возврат;
	КонецЕсли;
...
Процедура СообщитьУпр(Сообщение="", Статус=Неопределено)
	ТекстСообщенияПользователю=ТекстСообщенияПользователю+?(ПустаяСтрока(ТекстСообщенияПользователю), "", Символы.ПС)+Сообщение;
КонецПроцедуры//СообщитьУпр
