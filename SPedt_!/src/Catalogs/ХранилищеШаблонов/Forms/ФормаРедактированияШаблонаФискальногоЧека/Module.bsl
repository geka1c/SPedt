...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОшибкаЗагрузкиЭталонногоШаблона Тогда
		ТекстОшибки = "Ошибка загрузки эталонного шаблона";
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ПараметрЗакрытия = Неопределено И Модифицированность Тогда
		Ответ = Вопрос("Шаблон был изменен. Сохранить изменения?", РежимДиалогаВопрос.ДаНетОтмена, , КодВозвратаДиалога.Да);
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДПриходныйКассовыйОрдерПоляШаблона"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДПриходныйКассовыйОрдерПоляШаблонаШапка"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДПриходныйКассовыйОрдерПоляШаблонаТЧ"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДРасходныйКассовыйОрдерПоляШаблона"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДРасходныйКассовыйОрдерПоляШаблонаТЧ"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДРасходныйКассовыйОрдерПоляШаблонаШапка"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДЧекККМПоляШаблона"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДЧекККМПоляШаблонаТЧ_Товары"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДЧекККМПоляШаблонаШапка"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДЧекККМПоляШаблонаПодарки"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДЧекККМПоляШаблонаТЧ_Подарки"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "КомпоновщикНастроекСКДЧекККМПоляШаблонаШапкаПодарки"
...
///////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ЭЛЕМЕНТОВ ТАБЛИЧНОЙ ЧАСТИ "ШаблонЧека"
...
Процедура ШаблонЧекаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОтветФормы = ОткрытьФормуМодально("Справочник.ХранилищеШаблонов.Форма.ФормаРедактированияПараметров", СтруктураПараметров, ЭтаФорма);
...
Процедура ЗагрузитьШаблонПоУмолчанию(Команда)
	...
	Ответ = Вопрос("Текущий шаблон будет заменен на шаблон по умолчанию?", РежимДиалогаВопрос.ДаНет);
...
Процедура ОчиститьШаблон(Команда)
	Ответ = Вопрос("Очистить текущий шаблон?", РежимДиалогаВопрос.ДаНет);
...
Процедура ДобавитьВШаблонСтрокуТабличногоТипаИзМеню(Команда)
	...
	ДобавитьПользовательскуюСтрокуВШаблонЧека("Строка таблицы", "Таблица", Истина);
...
Процедура ДобавитьВШаблонСтрокуТабличногоТипа(Команда)
	...
	ДобавитьПользовательскуюСтрокуВШаблонЧека("Строка таблицы", "Таблица");
...
Процедура ДобавитьВШаблонСтрокуСоставногоТипаИзМеню(Команда)
	...
	ДобавитьПользовательскуюСтрокуВШаблонЧека("Составная строка", "СоставнаяСтрока", Истина);
...
Процедура ДобавитьВШаблонСтрокуСоставногоТипа(Команда)
	...
	ДобавитьПользовательскуюСтрокуВШаблонЧека("Составная строка", "СоставнаяСтрока");
...
///////////////////////////////////////////////////////////////////////////////
// Формирование и отображение внешнего вида примера чека.

// Выполняет формирование изображения фискального чека по заданному шаблону
//
// Параметры
//  НовоеЗначениеМодифицированностиФормы - <Булево> - Значение Истина означает, что процедура вызвана при изменении реквизитов влияющих на изменение шаблона чека.
//
&НаСервере
...
///////////////////////////////////////////////////////////////////////////////
// Импорт, экспорт, шаблон по умолчанию, очистка, сохранение шаблона чека.
...
//
// Параметры
//  ПанельКонтекстноеМеню - <КоманднаяПанель> - Панель или контекстное меню для команд которых устанавливается доступность.
...
Процедура ПолучитьСписокОшибочныхПолейСКД(Шаблон, Список, КомпоновщикНастроек)
	...
	Для каждого Строка Из Шаблон.Строки Цикл
		Если Строка.ТипЭлемента = "СтрокаДанных" Тогда
			ПолеСКД = УправлениеШаблонами.НайтиПолеСКДПоПолномуИмени(КомпоновщикНастроек.Настройки.Выбор.ДоступныеПоляВыбора.Элементы, СтрЗаменить(Строка.Элемент,"%%",""));
...
Процедура ДобавитьСтрокуВШаблонЧека(ИмяЭлемента, ТипЭлемента, Знач КоллекцияСтрок = Неопределено, ИмяКолонки = "")
	...
	Если ТипЭлемента = "СтрокаДанных" Тогда
		ПолеСКД = УправлениеШаблонами.НайтиПолеСКДПоПолномуИмени(КомпоновщикНастроек.Настройки.ДоступныеПоляПорядка.Элементы, СтрЗаменить(ИмяЭлемента,"%%",""));
